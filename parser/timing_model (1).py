...
class LutTable:
    ...
    def interpolate(self, slew: float, load: float) -> float:
        ...
        Some vendor Liberty exports have index_1/index_2 breakpoints that don't perfectly
        match the values() matrix dimensions (an export-tool quirk, not a Sky130-only issue --
        seen across GF180MCU, Sky130HD, and IHP SG13G2 files too). Rather than raising and
        crashing an entire library scan / CSV export over one malformed table, return NaN
        for that single cell and let the rest of the library process normally.
        """
        try:
...
def _bilinear(rows: list[float], cols: list[float], grid: list[list[float]],
    ...
    # Defensive clamp: some vendor Liberty exports declare index_1/index_2
    # breakpoints that don't exactly match the values() matrix dimensions
    # (an off-by-one row/column, or a table-splitting edge case). Rather than
    # raising IndexError on a single malformed cell and killing the whole
    # export, clamp to the actual matrix bounds we have.
    n_rows = len(grid)
    if n_rows == 0 or any(len(r) == 0 for r in grid):
        raise ValueError("empty or malformed LUT grid")

    r0 = min(r0, n_rows - 1)
    r1 = min(r1, n_rows - 1)
    n_cols_r0 = len(grid[r0])
    n_cols_r1 = len(grid[r1])
    c0_r0 = min(c0, n_cols_r0 - 1)
    c1_r0 = min(c1, n_cols_r0 - 1)
    c0_r1 = min(c0, n_cols_r1 - 1)
    c1_r1 = min(c1, n_cols_r1 - 1)
...
def _build_pin(pin_group: LibGroup) -> Pin:
    pin = Pin(
        name=pin_group.name,
        direction=pin_group.attrs.get("direction", "").strip().strip('"'),
